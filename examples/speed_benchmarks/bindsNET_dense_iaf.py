...
fl_input_processor = ArrayInputProcessor(
    {'I':
        {'uids': ['neuron{}'.format(i) for i in range(N)],
         'data': dense_input
        }
    })
fl_output_processor = [OutputRecorder([('spike_state', ['neuron{}'.format(i) for i in range(N)]),
                                       ('V', ['neuron{}'.format(i) for i in range(N)])],
                                       sample_interval=1)]
