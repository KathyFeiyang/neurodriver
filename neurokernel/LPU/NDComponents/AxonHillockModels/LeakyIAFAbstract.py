...
class LeakyIAFAbstract(BaseAxonHillockModel):
    # updates are the output states of the model
    updates = ['self_spike_state', # (bool)
               'V' # Membrane Potential (mV)
              ]
    # accesses are the input variables of the model
    accesses = ['spike_state']
    ### Fake input parameter for connecting to LeakyIAF, to mock dense inter-connections.
    ...
    def pre_run(self, update_pointers):
        super(LeakyIAFAbstract, self).pre_run(update_pointers)
...
__global__ void update(int num_comps,
               %(dt)s dt,
               int nsteps,
               %(input_spike_state)s* g_spike_state, // accesses
               ...
               %(update_self_spike_state)s* g_self_spike_state, //updates
               %(update_V)s* g_V)
{
    ...
    %(update_self_spike_state)s spike;
    ...
    for(int i = tid; i < num_comps; i += total_threads)
    {
        ...
        {
            V = V*bh + (resistance*spike_state+resting_potential)*(1.0 - bh);
            if (V >= threshold)
            {
                V = reset_potential;
                spike = 1.0;
